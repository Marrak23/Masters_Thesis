...
alphaHH = Conv.Froma0toA(alphaHH);
ReqHH   = Conv.FromAtoa0(ReqHH);
DeHH    = Conv.FromeVtoHartree(DeHH);

% *** Masses and vib. frequencies
m = [mF mH mH]/SI.m_e;   % In a.u.
mu = [muHF muHH muHF]/SI.m_e;
...
x0 = [42*1E-10/SI.a0, ReqHH];

n0      = 0;


D_e     = DeHH;                               % Depth of the potential well
beta    = alphaHH;                               % Morse potential parameter
...
existingData = readmatrix('Vib0FHH.txt');
...
Ebond3 = @(R1, R2)      DeHF*(exp(-2*alphaHF*(R1 - R2*mu(3)/(mu(2)+mu(3)) + R2 - ReqHF)) - 2*exp(-alphaHF*(R1 - R2*mu(3)/(mu(2)+mu(3)) + R2 - ReqHF)));
Eanti3 = @(R1, R2)  1/2*DeHF*(exp(-2*alphaHF*(R1 - R2*mu(3)/(mu(2)+mu(3)) + R2 - ReqHF)) + 2*exp(-alphaHF*(R1 - R2*mu(3)/(mu(2)+mu(3)) + R2 - ReqHF)));
...
W = JAB(10000,ReqHH) + JBC(ReqHH) + JAC(10000, ReqHH) - sqrt(1/2*((KAB(10000,ReqHH) - KBC(ReqHH))^2 + (KBC(ReqHH) - KAC(10000, ReqHH))^2 + (KAC(10000, ReqHH) - KAB(10000,ReqHH))^2));
...
writematrix(existingData, 'Vib0FHH.txt', 'Delimiter', '\t');
